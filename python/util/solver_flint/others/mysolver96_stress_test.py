import logging, time, argparse, csv, threading , multiprocessing
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        #print("total_threads_time:", total_threads_time)
        
        times.append(total_threads_time)
    average_time = sum(times)/len(times)
    print("average time for 10 runs:", thread_count)


    #exactly same but using multi processing 
    initial_time = time.time()

    all_client_threads = []
    thread_count = 0 

    while ((time.time() - initial_time) < 0.05):
        handle_peer_thread = multiprocessing.Process(target=solve, args=(dc_sums,))
        handle_peer_thread.start()
        all_client_threads.append(handle_peer_thread)
        thread_count += 1 
